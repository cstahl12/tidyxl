...
#'
#' @return
#' A nested list of vectors starting at the top level with `$style` and
#' `$local`, then drilling down to the vectors that hold the definitions.  E.g.
#' `my_formats$local$font$size`.
#'
#' @details
#' Cell formatting is returned by [tidyxl::xlsx_formats()].  There are two types
...
#' returned in the `$style` and `$local` sublists of `xlsx_formats()`, with
